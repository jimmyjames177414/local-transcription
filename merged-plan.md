# Plan: Claude Code CLI Workspace Provider (merged)

Merged from `opus-plan.md` (base) + `sonnet-plan.md` (transcript grounding).
Anchors verified against the code on 2026-07-14.
...
(verified in `LocalTranscriber.Voice.csproj`), so putting a backend in `Agent` that implements a
...
- **Transcript grounding (from sonnet):** before building the prompt in `RunTurnAsync`, if a
  transcript `.jsonl` path is configured:
  ...
  3. **Reuse the existing `TranscriptEventDeduplicator`** (`TranscriptEventTailer.cs:259`, reachable
     ...
     new — do **not** hand-roll timestamp tracking. It dedupes by a content-derived SHA256 id, so it
     is robust to non-monotonic or colliding timestamps. Hold one deduplicator per session; feed each
     parsed event through `TryAdd` and prepend only those it reports as new, as:
     ```
     [Recent transcript]
     <Speaker>: <text>
     ...
...
- **Settings** — `src/LocalTranscriber.App/Views/SettingsView.xaml`, "Assistant & privacy"
  section:
...
- **Cancel-turn affordance (backs Fix #4):** a Cancel button (or repurposed Stop) bound to a
  `CancelTurnCommand`, visible/enabled only while `State == Thinking`, that triggers the backend's
  per-turn cancellation. Without this the mid-turn kill path has no way to be invoked from the UI.
...
3. **Manual app test** (`./scripts/run-app.ps1` or F5; watch with `./scripts/tail-logs.ps1`):
   - Settings → Assistant & privacy: Provider = claude-cli, Browse… a workspace. **Use a throwaway
     ...
     the workspace's source, and you do not want it editing this app mid-test. (Once flows are trusted,
     a deliberate self-workspace run is fine, eyes-open.)
...
Both plans build the same thing: a Claude Code CLI backend that implements IRealtimeVoiceConversation so it drops into the existing chat/voice UI. I verified their key claims against the code.

Dimension: Assembly placement
sonnet-plan: ❌ Puts backend in LocalTranscriber.Agent, which only
references Shared — can't implement the Voice interface without an
unmentioned, dependency-inverting project ref
opus-plan: ✅ Puts it in LocalTranscriber.Voice, where the interface lives
────────────────────────────────────────
Dimension: Consent / permissions
sonnet-plan: ❌ Silent — CLI could edit files/run commands ungated,
violating the repo's consent-first rule
opus-plan: ✅ Gated behind existing VoiceModeConsentDialog; read-only
--tools otherwise
────────────────────────────────────────
Dimension: Cited anchors
sonnet-plan: ✅ Real (SafePathValidator, TranscriptEventTailer) but thin
opus-plan: ✅ Verified, incl. commit 7ec4ea5 actually removing
AgentProviderFactory.cs
────────────────────────────────────────
Dimension: Test story
sonnet-plan: ❌ None
opus-plan: ✅ Injectable process seam, fixture-based tests
────────────────────────────────────────
Dimension: Transcript grounding
sonnet-plan: ✅ Feeds recent transcript per turn (core for a meeting
copilot)
opus-plan: ❌ Explicitly deferred as out of scope
────────────────────────────────────────
Dimension: Simplicity
sonnet-plan: ✅ Simpler --output-format json, one-shot
opus-plan: Streaming stream-json — better UX, more parsing

Winner: opus-plan — correct where sonnet is architecturally broken (assembly placement), compliant where sonnet is silent (consent, a hard rule here), and more rigorous (test seam, verified anchors, honest unknowns).

Sonnet's one real edge: transcript grounding — genuinely important for a "live meeting copilot," which opus wrongly punted.

What the merged plan does

Opus as the base, with sonnet's transcript grounding promoted to a first-class v1 feature:
- Backend in LocalTranscriber.Voice (correct assembly), implementing IRealtimeVoiceConversation.
- stream-json parsing, session resumption (--session-id → --resume), consent-gated bypassPermissions vs read-only --tools.
- Sonnet's per-turn transcript snapshot (MaxTranscriptEvents, timestamp-dedupe via TranscriptEventJsonParser.TryParse) folded into RunTurnAsync.
- SafePathValidator on the workspace path, injectable process seam for tests, provider/workspace UI, optional CLI parity.

All line references (StartVoiceAsync:383, PersistConfig:657, hybrid STT branch 314-365, gating :72) are verified against the current code.